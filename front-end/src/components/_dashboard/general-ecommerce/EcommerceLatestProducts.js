...
import { Box, Link, Card, CardHeader, Typography, TextField, Stack } from '@material-ui/core';
// utils
...
export default function EcommerceLatestProducts({ getListCupon }) {
  const { enqueueSnackbar } = useSnackbar();

  const NewProductSchema = Yup.object().shape({
    name: Yup.string().required('Name is required'),
    value: Yup.string().required('Value is required'),
    cuponId: Yup.string().required('Cupon code is required')
  });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm, setSubmitting, setErrors }) => {
      try {
        const { success } = await axiosClient.post('/cupons/add', {
          ...values
        });
  ...
  return (
    <Card>
      <CardHeader title="Add Cupon" />
      <Scrollbar>
        <Stack spacing={3} sx={{ p: 3, pr: 0 }}>
          <FormikProvider value={formik}>
            <Form noValidate autoComplete="off" onSubmit={handleSubmit}>
              <Stack spacing={3} bottom={'20px'}>
                <TextField
                  fullWidth
                  label="Name"
                  {...getFieldProps('name')}
                ...
                <TextField
                  fullWidth
                  label="Cupon Code"
                  {...getFieldProps('cuponId')}
                  error={Boolean(touched.cuponId && errors.cuponId)}
                  helperText={touched.cuponId && errors.cuponId}
                />
              </Stack>
